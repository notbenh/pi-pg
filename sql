...
DROP VIEW v_product_subject;
DROP TABLE IF EXISTS r_product_subject CASCADE;
...
CREATE TABLE meta_type( id bigserial not null,meta_type varchar(255) not null UNIQUE, PRIMARY KEY (id));
...
CREATE TABLE subject( id bigserial not null,subject varchar(255) not null UNIQUE, PRIMARY KEY (id));
...
CREATE TABLE r_product_meta_data( id bigserial not null,product_id bigint REFERENCES product (id) ON DELETE RESTRICT ON UPDATE CASCADE ,meta_type_id bigint REFERENCES meta_type (id) ON DELETE RESTRICT ON UPDATE CASCADE ,meta_data_id bigint REFERENCES meta_data (id) ON DELETE RESTRICT ON UPDATE CASCADE ,UNIQUE (product_id, meta_data_id, meta_type_id, data_source_instance_id),data_source_instance_id bigint REFERENCES data_source_instance (id) ON DELETE RESTRICT ON UPDATE CASCADE ,timestamp timestamp with time zone not null DEFAULT current_timestamp,trust smallint CHECK( 100 >= trust AND trust >= 0 )  DEFAULT 1, PRIMARY KEY (id));
...
CREATE TABLE r_product_subject( id bigserial not null,product_id bigint REFERENCES product (id) ON DELETE RESTRICT ON UPDATE CASCADE ,subject_id bigint REFERENCES subject (id) ON DELETE RESTRICT ON UPDATE CASCADE ,data_source_instance_id bigint REFERENCES data_source_instance (id) ON DELETE RESTRICT ON UPDATE CASCADE ,timestamp timestamp with time zone not null DEFAULT current_timestamp,trust smallint CHECK( 100 >= trust AND trust >= 0 )  DEFAULT 1, PRIMARY KEY (id));
...
CREATE TABLE r_product_meta_date( id bigserial not null,product_id bigint REFERENCES product (id) ON DELETE RESTRICT ON UPDATE CASCADE ,meta_type_id bigint REFERENCES meta_type (id) ON DELETE RESTRICT ON UPDATE CASCADE ,meta_date_id bigint REFERENCES meta_date (id) ON DELETE RESTRICT ON UPDATE CASCADE ,UNIQUE (product_id, meta_date_id, meta_type_id, data_source_instance_id),data_source_instance_id bigint REFERENCES data_source_instance (id) ON DELETE RESTRICT ON UPDATE CASCADE ,timestamp timestamp with time zone not null DEFAULT current_timestamp,trust smallint CHECK( 100 >= trust AND trust >= 0 )  DEFAULT 1, PRIMARY KEY (id));
CREATE VIEW v_product_c_blurb AS
                             SELECT r_product_c_blurb.id, product.pid, c_blurb.id AS blurb, r_product_c_blurb.timestamp, trust * calc_trust(r_product_c_blurb.data_source_instance_id) as trust
...
CREATE VIEW v_currency_currency_display AS
                             SELECT r_currency_currency_display.id, currency.currency, currency_display.currency_display, r_currency_currency_display.timestamp, trust * calc_trust(r_currency_currency_display.data_source_instance_id) as trust
...
CREATE VIEW v_product_copyright AS
                             SELECT r_product_copyright.id, product.pid, copyright.copyright, r_product_copyright.timestamp, trust * calc_trust(r_product_copyright.data_source_instance_id) as trust
...
CREATE VIEW v_c_blurb_meta_date AS
                             SELECT r_c_blurb_meta_date.id, c_blurb.id AS blurb, meta_date.meta_date, r_c_blurb_meta_date.timestamp, trust * calc_trust(r_c_blurb_meta_date.data_source_instance_id) as trust
...
CREATE VIEW v_product_binding AS
                             SELECT r_product_binding.id, product.pid, binding.binding, r_product_binding.timestamp, trust * calc_trust(r_product_binding.data_source_instance_id) as trust
...
CREATE VIEW v_product_meta_data AS
                             SELECT r_product_meta_data.id, product.pid, meta_type.meta_type, meta_data.meta_data, r_product_meta_data.timestamp, trust * calc_trust(r_product_meta_data.data_source_instance_id) as trust
                             FROM r_product_meta_data
                             JOIN product ON (r_product_meta_data.product_id = product.id)
                             JOIN meta_type ON (r_product_meta_data.meta_type_id = meta_type.id)
                             JOIN meta_data ON (r_product_meta_data.meta_data_id = meta_data.id)
;
                            
CREATE VIEW v_blurb_type_blurb_type_display AS
                             SELECT r_blurb_type_blurb_type_display.id, blurb_type.blurb_type, blurb_type_display.blurb_type_display, r_blurb_type_blurb_type_display.timestamp, trust * calc_trust(r_blurb_type_blurb_type_display.data_source_instance_id) as trust
...
CREATE VIEW v_c_blurb_meta_data AS
                             SELECT r_c_blurb_meta_data.id, c_blurb.id AS blurb, meta_data.meta_data, r_c_blurb_meta_data.timestamp, trust * calc_trust(r_c_blurb_meta_data.data_source_instance_id) as trust
...
CREATE VIEW v_binding_binding_display AS
                             SELECT r_binding_binding_display.id, binding.binding, binding_display.binding_display, r_binding_binding_display.timestamp, trust * calc_trust(r_binding_binding_display.data_source_instance_id) as trust
...
CREATE VIEW v_product_language AS
                             SELECT r_product_language.id, product.pid, language.language, r_product_language.timestamp, trust * calc_trust(r_product_language.data_source_instance_id) as trust
...
CREATE VIEW v_product_list_price AS
                             SELECT r_product_list_price.id, product.pid, list_price.list_price, r_product_list_price.timestamp, trust * calc_trust(r_product_list_price.data_source_instance_id) as trust
...
CREATE VIEW v_product_subject AS
                             SELECT r_product_subject.id, product.pid, subject.subject, r_product_subject.timestamp, trust * calc_trust(r_product_subject.data_source_instance_id) as trust
                             FROM r_product_subject
                             JOIN product ON (r_product_subject.product_id = product.id)
                             JOIN subject ON (r_product_subject.subject_id = subject.id)
;
                            
CREATE VIEW v_product_title AS
                             SELECT r_product_title.id, product.pid, title.title, r_product_title.timestamp, trust * calc_trust(r_product_title.data_source_instance_id) as trust
...
CREATE VIEW v_language_language_display AS
                             SELECT r_language_language_display.id, language.language, language_display.language_display, r_language_language_display.timestamp, trust * calc_trust(r_language_language_display.data_source_instance_id) as trust
...
CREATE VIEW v_product_meta_date AS
                             SELECT r_product_meta_date.id, product.pid, meta_type.meta_type, meta_date.meta_date, r_product_meta_date.timestamp, trust * calc_trust(r_product_meta_date.data_source_instance_id) as trust
                             FROM r_product_meta_date
                             JOIN product ON (r_product_meta_date.product_id = product.id)
                             JOIN meta_type ON (r_product_meta_date.meta_type_id = meta_type.id)
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'author'), (SELECT id FROM product WHERE pid = '9780803735330'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'author') AND meta_data = 'Jerry Pinkney'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'edition Description'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'edition Description') AND meta_data = 'AMERICAN Paperback'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'publication Date'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'publication Date') AND meta_data = 'September 1999'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'dimensions'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'dimensions') AND meta_data = '7.64x5.25x.78 in. .53 lbs.'));
INSERT INTO meta_type (meta_type) VALUES ('author');
INSERT INTO meta_data (meta_type_id, meta_data) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'author'), 'J. K. Rowling');
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'author'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'author') AND meta_data = 'J. K. Rowling'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'series'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'series') AND meta_data = 'Harry Potter (Paperback)'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'series Volume'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'series Volume') AND meta_data = '01'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'publisher'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'publisher') AND meta_data = 'Arthur A. Levine Books'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'edition Number'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'edition Number') AND meta_data = '1st American ed.'));
...
INSERT INTO subject (subject) VALUES ('Fiction');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Fiction'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Science Fiction, Fantasy, & Magic');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Science Fiction, Fantasy, & Magic'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Humorous Stories');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Humorous Stories'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Childrens 9-12 - Fiction - Fantasy');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Childrens 9-12 - Fiction - Fantasy'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Schools');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Schools'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('School & Education');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'School & Education'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('England');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'England'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Magic');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Magic'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Wizards');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Wizards'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Witches');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Witches'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('England Fiction.');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'England Fiction.'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Potter, Harry');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Potter, Harry'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
INSERT INTO subject (subject) VALUES ('Fantasy & Magic');
INSERT INTO r_product_subject (subject_id, product_id, data_source_instance_id) VALUES ((SELECT id FROM subject WHERE subject = 'Fantasy & Magic'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'age Level'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'age Level') AND meta_data = '08-12'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'illustrations'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'illustrations') AND meta_data = 'Y'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'illustrator'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'illustrator') AND meta_data = 'Grandpre, Mary'));
...
INSERT INTO r_product_meta_data (meta_type_id, product_id, data_source_instance_id, meta_data_id) VALUES ((SELECT id FROM meta_type WHERE meta_type = 'pages'), (SELECT id FROM product WHERE pid = '9780590353427'), (SELECT id FROM data_source_instance WHERE data_source_instance = 'inserter'), (SELECT id FROM meta_data WHERE meta_type_id = (SELECT id FROM meta_type WHERE meta_type = 'pages') AND meta_data = '312'));
